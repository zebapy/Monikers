...
var aboutToStartTurnHtml =
  "<p>%roundname%</p><p>Choose someone from %nextteam%</p><p>Cards Gussed: %guessedCount%</p><p>Cards Passed: %passedCount%</p><p>Cards Left In Deck: %deckCount%</p>";
var roundOverHtml =
  "<p>Round %roundnum% is over!</p><p>%winningmsg%</p><p>%team1%: %1teampoints%</p><p>%team2%: %2teampoints%</p><br><p>%roundname%</p><p>Cards in Deck: %deckCount%</p><p>Choose someone from %nextteam%</p>";
var aboutToStartGameHtml =
  "<p>%roundname%</p><p>Cards in Deck: %deckCount%</p><p>Choose someone from %nextteam%</p>";
var gameOverHtml =
  "<p>Game Over!</p><p>%winningmsg%</p><p>%team1%: %1teampoints%</p><p>%team2%: %2teampoints%</p>";
var button =
  '<button type="button" onClick="startTurn()" id="startButton"><p>Start Round</p></button>';
var restartbutton =
  '<button type="button" onClick="restart()" id="startButton"><p>Restart</p></button>';
var roundNames = [
  "Round 1: No Restrictions",
  "Round 2: One Word",
  "Round 3: Only Gestures",
];
...
$("document").ready(function () {
  ...
  loadCards(function (cards) {
    ...
    if (localStorage.getItem("deck") != null) {
      game = new Game(
        parseInt(localStorage.getItem("cardNum")),
        3,
        localStorage.getItem("blueName"),
        localStorage.getItem("redName"),
        allCards.length
      );
      game.restoreState();
      if (localStorage.getItem("currentPhase") == "newround") roundOver(true);
      else if (localStorage.getItem("currentPhase") == "newgame")
        gameAboutToStart();
      else turnAboutToStart();
    }
  });
});

function newGame(numCards, team1Name, team2Name) {
  game = new Game(numCards, 3, team1Name, team2Name, allCards.length);
  game.init();
}
...
function correct() {
  if (game.correct()) {
    ...
    if (next == -1) {
      ...
      document.getElementById("card").innerHTML = cardToImg(next);
    }
  }
}
...
function pass() {
  if (game.pass()) {
    var next = game.nextCard();
    if (next == -1) roundOver();
    else document.getElementById("card").innerHTML = cardToImg(next);
  }
}
...
function undo() {
  if (game.undo())
    document.getElementById("card").innerHTML = cardToImg(game.currentCard);
}
...
function startTurn() {
  if (game == null) {
    ...
    if (new Date().getTime() - timeWhenTurnEnded < 2000) return;

    $("#cardContainer").empty();
    $("#cardContainer").html(
      '<div id="card">' + cardToImg(game.nextCard()) + "</div>"
    );
...
function cardToImg(cardNum) {
  var card = allCards[cardNum];

  if (!card) throw new Error("No card found for cardNum " + cardNum);

  var c = card;

  return `<div>
<p class='card-cat'>${c.category}</p>
