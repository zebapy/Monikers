...
function Game(cardNum, numRounds, team1Name, team2Name, cardsTotal) {
  console.log({
    cardNum,
    numRounds,
    team1Name,
  ...
  this.init = function () {
    ...
    while (deck.length < this.cardNum) {
      var random = Math.ceil(Math.random() * cardsTotal);
      if (deck.indexOf(random) == -1) deck.push(random);
    }
    this.deck = shuffleArray(deck);

    if (Math.random() < 0.5) this.currentTurn = -1;
    else this.currentTurn = 1;
  };
  this.getNextTeam = function () {
    if (this.currentTurn == 1) return this.blueName;
    else return this.redName;
  };
  this.getGameStatus = function () {
    if (this.redPoints > this.bluePoints) return this.redName + " is winning!";
  ...
  this.getFinalGameStatus = function () {
    if (this.redPoints > this.bluePoints) return this.redName + " won!";
    else if (this.bluePoints > this.redPoints) return this.blueName + " won!";
  ...
  this.pass = function () {
    if (this.currentCard == -1) return false;
  ...
  this.nextCard = function () {
    ...
    if (this.undoInfo != null) this.undoInfo.push(this.currentCard);
  ...
  this.undo = function () {
    if (this.undoInfo == null) return false;
  ...
  this.startTurn = function () {
    ...
    else this.cardsInPileAtStartOfTurn = this.redPile.length;
  };
  this.endTurn = function () {
    ...
    if (this.currentTurn == 1)
      this.cardsJustGotten =
        this.bluePile.length - this.cardsInPileAtStartOfTurn;
    else
      this.cardsJustGotten =
        this.redPile.length - this.cardsInPileAtStartOfTurn;
  ...
  this.nextRound = function () {
    ...
    this.redPile = [];
    this.bluePile = [];
    if (this.redPoints > this.bluePoints) this.currentTurn = 1;
    else if (this.bluePoints > this.redPoints) this.currentTurn = -1;
    else this.currentTurn = Math.random() < 0.5 ? 1 : -1;
  };
